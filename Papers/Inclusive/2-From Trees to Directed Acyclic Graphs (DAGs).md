...
### Deﬁnition 1. 
Given a set of transactions T, a transaction tx is consistent with T if all its inputs are outputs of transactions in T, and no other transaction in T uses them as inputs. We say that T is consistent, if every transaction tx ∈ T is consistent with T \ {tx}.
...
### Algorithm 1. 
Inclusive-F(G, B, T) Input: a DAG G, a block B with pointers to predecessors (B 1 , ..., B m ) (ordered according to ≺<sub>B</sub> ), <sup>7</sup> and a set of previously conﬁrmed transactions T.
